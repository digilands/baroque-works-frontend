...
import { useCategories, useCreateJob, useUpdateJob } from "@/hooks/useMarketplace";
...
const validationSchema = Yup.object({
  title: Yup.string().min(5, "At least 5 characters").max(100, "At most 100 characters").required("Title is required"),
  description: Yup.string().min(20, "At least 20 characters").max(2000, "At most 2000 characters").required("Description is required"),
  category: Yup.string().required("Category is required"),
  minBudget: Yup.number().typeError("Enter an amount").min(0, "Must be positive").required("Min budget is required"),
...
export default function JobCreateForm({
  ...
  const handleSubmit = async (values: JobFormValues) => {
    ...
    const payload = {
      ...
      location: { coordinates: [picked.longitude, picked.latitude] as [number, number] },
    };
    try {
      if (isEdit && jobId) {
        ...
        router.push(result.id ? `/dashboard/jobs/${result.id}` : "/dashboard/jobs");
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">{isEdit ? "Edit job" : "Post a job"}</h1>
      <p className="text-sm text-gray-500 mb-8">
        {isEdit ? "Update the details — pros see changes immediately." : "Describe the work — pros nearby can respond."}
      </p>
      ...
      <Formik<JobFormValues>
        initialValues={{
          ...
          minBudget: initial?.minBudget != null ? String(initial.minBudget) : "",
          maxBudget: initial?.maxBudget != null ? String(initial.maxBudget) : "",
      ...
      >
        {({ isSubmitting }) => (
          <Form className="space-y-5">
            <TextInput label="Job title" name="title" required placeholder="e.g. Fix leaking kitchen pipe" />
            <TextInput label="Description" name="description" required multiline rows={5} placeholder="What needs doing? Materials provided? Access notes?" />
            <SelectInput
              label="Category"
              name="category"
              required
              placeholder={catsLoading ? "Loading categories…" : "Select a category…"}
              options={categories.map((c) => ({ value: c._id, label: c.displayName }))}
            />
            <div className="grid grid-cols-2 gap-4">
              <TextInput label="Min budget (₦)" name="minBudget" required type="number" placeholder="5000" />
              <TextInput label="Max budget (₦)" name="maxBudget" required type="number" placeholder="15000" />
            </div>
            <SelectInput label="Urgency" name="urgency" required options={URGENCY_OPTIONS} />

            <div className="space-y-3">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                Job location <span className="text-red-500 ml-1 font-bold">*</span>
              </p>
              {!showMap ? (
                <button
                  type="button"
                ...
                >
                  {picked ? `${picked.label.slice(0, 52)} — change` : "Pin the job location on the map"}
                </button>
              ) : (
                <LocationPicker
                  initial={picked}
