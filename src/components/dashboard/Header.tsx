...
const Header: React.FC<HeaderProps> = ({ onMenuClick, onNotificationClick, title }) => {
  const { user } = useAuth();
  return (
    <header className="flex items-center justify-between gap-3 px-4 sm:px-6 py-4 bg-transparent mb-3 sm:mb-6 min-w-0">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-lg sm:text-xl font-semibold text-gray-500 truncate">{title}</h1>
      </div>

      <div className="flex items-center gap-2 sm:gap-4 shrink-0">
        {user?.role === "handyman" && <div className="hidden sm:flex items-center gap-2 bg-white rounded-full px-1 py-1 border border-gray-200">
